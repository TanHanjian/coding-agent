...
func (h *MemoryGenerationHub) PublishEvent(assistantMessageID string, event GenerationEvent) {
	...
	run.eventSummaries = append(run.eventSummaries, GenerationEvent{Kind: event.Kind, StepID: event.StepID, Phase: event.Phase, ToolCallID: event.ToolCallID, ToolName: event.ToolName, ToolTitle: event.ToolTitle, ErrorText: event.ErrorText})
